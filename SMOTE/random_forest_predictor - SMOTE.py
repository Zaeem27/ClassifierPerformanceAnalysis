...
file = open(rootdir+"/tables_forest_lucene.txt", "w")
...
for subdir, dirs, files in os.walk(rootdir):
    if path.exists(subdir+"/test.csv") and path.exists(subdir+"/train.csv"):     
        ...
        X_resampled, y_resampled = SMOTE().fit_resample(train_data, train_target)
        test_data_resampled, test_target_resampled = SMOTE().fit_resample(test_data, test_target)

        
        regr = RandomForestRegressor(max_features=None,max_depth=None,n_estimators=1000,min_samples_split=2)
        test_pred = regr.fit(X_resampled, y_resampled).predict(test_data_resampled)
